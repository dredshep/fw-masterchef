...
contract FSXU is ERC20Detailed ,Owned {
  ...
  constructor() public payable ERC20Detailed(tokenName, tokenSymbol, tokenDecimals) {
    _mint(0xf1aE41395619fa69bB5AaaDa2E4c5b13b6353812, _totalSupply);
  }
...
contract MasterChef is Ownable {
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, IBEP20 _rewardToken, bool _withUpdate) public onlyOwner {
    ...
    function safeFsxuTransfer(address _to, uint256 _amount) internal {
        uint256 fsxuBal = fsxu.balanceOf(address(this));
        if (_amount > fsxuBal) {
            fsxu.transferFrom(address(fsxu), _to, fsxuBal);
        } else {
            fsxu.transferFrom(address(fsxu), _to, _amount);
        }
    }
    ...
    function safeWhirlTransfer(address _to, uint256 _amount) internal {
        uint256 whirlBal = whirl.balanceOf(address(this));
        if (_amount > whirlBal) {
            whirl.transferFrom(address(whirl), _to, whirlBal);
        } else {
            whirl.transferFrom(address(whirl), _to, _amount);
        }
    }
